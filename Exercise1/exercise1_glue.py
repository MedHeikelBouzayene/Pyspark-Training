"""
This module create a dataset by joining multiple DataFrames stored in AWS S3, 
and then upload the result in S3
"""
...
class DF:
    """
    This class represents a spark DataFrame
    """
    ...
    def __init__(self, spark, path, schema=None):
        if schema:
            ...
            self.df = spark.read.options(header="True", inferSchema="True").parquet(
                path
            )
...
class DataSet:
    def __init__(self, dfs, keys):
        ...
        for i in range(1, len(dfs)):
            #   save common columns that are not keys to remove them after
            #   the join operation
            columns_intersection = self.df.columns_intersection(dfs[i])
            columns_intersection_and_not_keys = [
                dfs[i].df[x] for x in columns_intersection if (x not in keys[i - 1])
            ]
...
DATASET_SCHEMA: StructType = StructType(
    [
        ...
        StructField("M_ORIG", LongType()),  # if we use FloatType,
        # we have an error cuz it is
        # registered as INT64 in the
        # parquet file metadata.
        StructField("M_ORIG_SHAR", LongType()),  # if we use FloatType,
        # we have an error cuz it is
        # registered as INT64 in the
        # parquet file metadata.
...
MAAG_MASTER_AGREEMENT_SCHEMA: StructType = StructType(
    [
        StructField("C_ACT_TYPE", StringType()),
        # if we use StringType,
        # we have an error cuz it is
        # registered as Date in the
        # parquet file metadata.
        StructField("D_CRDT_COMMITTEE_APRV", DateType()),
        StructField("CENMES", IntegerType()),
        StructField("L_MAST_AGREM_NAME", StringType()),
...
MAAG_RATY_LINKED_SCHEMA: StructType = StructType(
    [
        ...
        # registered as INT64 in the
        # parquet file metadata.
        StructField("M_ORIG", LongType()),
        # if we use FloatType,
        # we have an error cuz it is
        # registered as INT64 in the
        # parquet file metadata.
        StructField("M_ORIG_SHAR", LongType()),
        StructField("C_MAST_AGREM_REFER", StringType()),
        StructField("N_APPLIC_INFQ", IntegerType()),
    ]
)
...
if __name__ == "__main__":
    print("test jenkins46")
    spark = SparkSession.builder.appName("Exercice 1-Read-Write-From-S3").getOrCreate()
    ...
    dataset = DataSet(
        [
            maag_master_agremDF,
            reac_ref_act_typeDF,
            maag_repa_rrol_linkedDF,
            maag_raty_linkedDF,
            rtpa_ref_third_partyDF,
        ],
        [
